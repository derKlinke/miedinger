...
exports.findJustfile = findJustfile;
exports.maybeUpdateJustfile = maybeUpdateJustfile;
const fs = require("fs");
const path = require("path");
function findJustfile(targetPath) {
...
function formatRecipeLines(presets) {
    if (presets.size === 0) {
        return [];
    }
    const lines = [];
...
function buildFormatBlock(presets) {
...
function findFormatBlockRange(lines) {
    let start = -1;
    let end = -1;
...
function findFormatTargetRange(lines) {
    const isAttribute = (line) => /^\s*\[.*\]\s*$/.test(line);
    const isRecipe = (line) => /^\s*[^#\s].*:$/.test(line.trimEnd()) && !isAttribute(line);
    let start = -1;
    for (let idx = 0; idx < lines.length; idx += 1) {
        const line = lines[idx];
...
function removeFormatTargets(lines) {
    let current = [...lines];
    while (true) {
...
function updateJustfileContent(content, block) {
    if (block.length === 0) {
        return null;
    }
...
function maybeUpdateJustfile(targetPath, presets, justMode) {
    if (justMode === "skip") {
        return null;
    }
    const recipe = formatRecipeLines(presets);
