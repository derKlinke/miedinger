...
function resolveConfigDir(): { dir: string } {
  ...
  const entries = fs.readdirSync(tmpDir).filter((entry: string) => entry.startsWith("format-configs-"));
  if (entries.length === 0) {
  ...
  return new Promise((resolve) => {
    ...
    rl.question("", (answer: string) => {
      ...
      const tokens = answer
        ...
        .filter((value: number) => Number.isFinite(value) && value >= 1 && value <= choices.length)
